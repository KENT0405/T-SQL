...
DECLARE 
	 @DB_name		VARCHAR(30)  = 'fs_egame_ticket'
...
BEGIN
	...
	FROM DISK = N''D:\BAK\ticket\full\' + @BakFile_full + '.bak''
	WITH FILE = 1, ' 
...
BEGIN
	SET @SQL += N'
	RESTORE DATABASE [' + @DB_name + '] 
	FROM DISK = N''D:\BAK\ticket\diff\' + @BakFile_diff + '.bak''
	WITH FILE = 1, ' 
...
BEGIN
	SET @SQL += N'
	RESTORE LOG [' + @DB_name + '] 
	FROM DISK = N''D:\BAK\ticket\log\' + @BakFile_log1 + '.bak''
	WITH FILE = 1, '
...
BEGIN
	SET @SQL += N'
	RESTORE LOG [' + @DB_name + '] 
	FROM DISK = N''D:\BAK\ticket\log\' + @BakFile_log2 + '.bak''
	WITH FILE = 1, '
...
BEGIN
	SET @SQL += N'
	RESTORE LOG [' + @DB_name + '] 
	FROM DISK = N''D:\BAK\ticket\log\' + @BakFile_log3 + '.bak''
	WITH FILE = 1, '
...
BEGIN
	SET @SQL += N'
	RESTORE LOG [' + @DB_name + '] 
	FROM DISK = N''D:\BAK\ticket\log\' + @BakFile_log4 + '.bak''
